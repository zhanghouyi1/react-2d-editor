...
export const transform = (direction, oriPos, e, txtDom, unitVector, target, opposite,editorOffset) => {
  const {
    top,
    left,
    cX,
  ...
  /**计算中心点 */
  const centerX = (left + editorOffset.current.left) + width / 2
  const centerY = (top+editorOffset.current.top) + height / 2
